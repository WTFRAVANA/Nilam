...
bot = telebot.TeleBot('7506989974:AAFGCHmAa7RABX23sg_px6wedNvlHjer78U')
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "You have not purchased yet purchase now from:- @wtf_ravana."
        ...
        try:
            with open(USER_FILE, "r+") as file:
                ...
                if log_content.strip() == "":
                    ...
                    file.truncate(0)
                    response = "users Cleared Successfully 🎖️"
        except FileNotFoundError:
            response = "users are already cleared 🌱."
    else:
        response = "you have to buy it from :- @wtf_ravana."
    bot.reply_to(message, response)
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝘼𝙩𝙩𝙖𝙘𝙠 𝙎𝙩𝙖𝙧𝙩𝙚𝙙🌬️🌬️\n\n𝙏𝙖𝙧𝙜𝙚𝙩: {target}\n𝙋𝙤𝙧𝙩: {port}\n𝙏𝙞𝙢𝙚: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝘽𝙮 𝙑𝙞𝙥 𝙐𝙨𝙚𝙧 𝙤𝙛- @wtf_ravana"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown 🌬️. Please Wait 10sec Before Running The /bgmi Command Again."
                bot.reply_to(message, response)
                return
        ...
        response = ("🌋 Unauthorized Access! 🌴\n\nOops! It seems like you don't have permission to use the /bgmi command. DM TO BUY ACCESS:- @@wtf_ravana")
...
def welcome_start(message):
    ...
    response = f'''🌅 JAY SURYDEV {user_name} WELCOME TO OUR DDOS PREMIUM SERVER BASED BOT 𝐚𝐭𝐭𝐚𝐜𝐤 𝐭𝐢𝐦𝐞 𝐥𝐢𝐦𝐢𝐭~ 10𝐦𝐢𝐧𝐮𝐭𝐞𝐬 .
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            message_to_broadcast = "🧿 𝙈𝙚𝙨𝙨𝙖𝙜𝙚 𝙩𝙤 𝙖𝙡𝙡 𝙪𝙨𝙚𝙧𝙨 𝙛𝙧𝙤𝙢 NIRMAL:\n\n" + command[1]
            with open(USER_FILE, "r") as file:
                user_ids = file.read().splitlines()
                for user_id in user_ids:
                    try:
